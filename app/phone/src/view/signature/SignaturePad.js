Ext.define('MyMobileApp.view.signature.SignaturePad', {
  extend: 'Ext.Component',
  requires: [
    'MyMobileApp.view.signature.Bezier',
    'MyMobileApp.view.signature.Point',
  ],
  xtype: 'signaturepad',
  ...
  listeners: {
    resize: function (self, width, height) {
      self.initCanvas();
      self._canvas.width = 400;
      self._canvas.height = 300;
      self.clear();
      self.fromDataURL(self._dataURL);
    },
  },

  html: '<canvas id="signature-pad" class="signature-pad"></canvas>',
  ...
  _createPoint: function (event) {
    ...
    return new MyMobileApp.view.signature.Point(
  ...
  _addPoint: function (point) {
    ...
    if (points.length > 2) {
      ...
      c3 = tmp.c1;
      curve = new MyMobileApp.view.signature.Bezier(
        points[1],
        c2,
        c3,
        points[2]
  ...
  _calculateCurveControlPoints: function (s1, s2, s3) {
    ...
    return {
      c1: new MyMobileApp.view.signature.Point(m1.x + tx, m1.y + ty),
      c2: new MyMobileApp.view.signature.Point(m2.x + tx, m2.y + ty),
    };
  },
